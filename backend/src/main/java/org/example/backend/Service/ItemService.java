...
public List<Item> updateItemsById(List<Item> items) {
    ...
    updatedItems.forEach(item -> {
        ...
        mongoItem.setModifiedTime(LocalDateTime.now());
        mongoItemRepository.save(mongoItem);
        try {
            String mongoItemJson = objectMapper.writeValueAsString(mongoItem);
            redisTemplate.opsForValue().set("mongoItem:" + item.getId(), mongoItemJson);
